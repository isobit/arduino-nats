...
public:
	...
	public:
		...
		inline void respond(String msg = "") __attribute__((always_inline)) {
			respond(msg.c_str());
		}

		void respond(const char* msg = "") {
			if (strlen(msg))
				nats->publish(reply, msg);
		}
	};
...
private:
	...
	inline void send(String msg) __attribute__((always_inline)) {
		send(msg.c_str());
	}
...
public:
	...
	inline void publish(const char* subject, String msg = "", const char* replyto = NULL) __attribute__((always_inline)) {
		return publish(subject, msg.c_str(), replyto);
	}
	...
	void publish(const char* subject, const char* msg = NULL, const char* replyto = NULL) {
		...
		send_fmt("PUB %s %s %lu",
			...
			(unsigned long)strlen((msg == NULL) ? "" : msg));
		send((msg == NULL) ? "" : msg);
	}

	inline void publish(const char* subject, const bool msg) __attribute__((always_inline)) {
	...
	inline int request(const char* subject, String msg, sub_cb cb, const int max_wanted = 1) __attribute__((always_inline)) {
		return request(subject, msg.c_str(), cb, max_wanted);
	}
